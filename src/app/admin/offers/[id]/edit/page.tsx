...
export default function OfferEditPage({ params }: OfferEditPageProps) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/offers/${params.id}`)
      ...
      .then((data) => {
        ...
        // Set selected products and their variant selections
        if (data.offerProducts) {
          const products = data.offerProducts.map((op: any) => op.product);
          setSelectedProducts(products);

          // Load existing variant selections
          const variantSelections = new Map<string, string[]>();
          data.offerProducts.forEach((op: any) => {
            if (op.selectedVariantOptionIds && op.selectedVariantOptionIds.length > 0) {
              variantSelections.set(op.productId, op.selectedVariantOptionIds);
            } else if (op.product.hasVariants && op.product.productVariants) {
              // If no variants selected but product has variants, select all by default
              const allVariantIds = op.product.productVariants.map((pv: any) => pv.variantOption.id);
              variantSelections.set(op.productId, allVariantIds);
            }
          });
          setProductVariantSelections(variantSelections);
        }
      })
      .catch((error) => {
        toast({
  ...
  return (
    <div>
      <div className="mb-8 flex items-center gap-4">
        <Link href={`/admin/offers/${params.id}`}>
        ...
        <div>
          ...
          <p className="text-gray-600">Update offer details and products</p>
        </div>
      </div>
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    URL-friendly identifier (e.g., buy-4-for-2000)
                  </p>
                </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                {/* Search & Add Products */}
                <div>
                  <Label htmlFor="search">Add More Products</Label>
